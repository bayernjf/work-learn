...
- [ ] 清债（可最后）：~~删两个空壳包~~（已删，commit `0b59fa3`/`40b862a`）、~~删死文件~~（同上）、~~拆 `main.tsx`~~（叶子组件已按域拆分，commit `6cc1ef0`，`App` 的状态/处理器抽 hooks 仍可做）、🟡 收敛双实现（接口层已完成：`d3bfca7` context 锚点 + `f1d5728` 同步面协议化；剩余 FK 语义统一，需本地表迁移）。
