...
| 项 | 状态 |
|----|------|
| 双实现（`direct.ts` 云端 vs `local-store` 本地）是最大技术债，FK/唯一约束/tombstone CHECK/触发器全在分叉 | 🟢 接口层已收敛：`d3bfca7`（`WorkLearnContext` 下沉 shared-schema，三端 context 编译期结构互检）+ `f1d5728`（同步面协议化：`LocalSyncStore`/`CloudSyncClient` 两端结构互检，编排收敛为共享 `runSync`）；剩余：本地表迁移统一 FK 语义（重活，另立专题） |
...
2. 推进顺序建议：`/register` 限流（安全面，需决策）→ FK 语义统一（需本地表迁移）。双实现收尾（接口 + 同步面）已完成，见上表。
