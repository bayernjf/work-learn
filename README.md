...
- 十三个 MCP 工具：`create_session`、`save_material`、`save_question_translation`、`search_corpus`、`get_review_items`、`mark_mastered`、`snooze_review`、`generate_practice`、`get_user_patterns`、`get_reuse_summary`、`record_reuse`、`suggest_reuse`；
...
测试：相关包检查全绿（`packages/shared-schema` 19 例；`packages/local-store` 20 例；`packages/mcp-server` 23 例；`apps/api` 鉴权相关测试保持独立运行）。
