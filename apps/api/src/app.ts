...
app.post("/practice/adaptive", async (c) => {
  const ctx = await contextFor(c.req.header("Authorization"));
  if (!ctx) return c.json({ error: "Unauthorized" }, 401);

  const parsed = generateAdaptivePracticeInputSchema.safeParse(await c.req.json().catch(() => ({})));
  if (!parsed.success) return c.json({ error: "Invalid adaptive practice request", issues: parsed.error.issues }, 400);

  try {
    return c.json({ data: await ctx.generateAdaptivePractice(parsed.data) });
  } catch (error) {
    return c.json(errorResponse("Could not generate adaptive practice", error));
  }
});
