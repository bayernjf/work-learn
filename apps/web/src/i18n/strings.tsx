...
6. Tell me to restart the client, then confirm these five tools are available: create_session, save_material, search_corpus, get_review_items, mark_mastered.`,
    ...
    step2: "3. Or run the local installer, then paste your token when it asks.",
    ...
    hint2b: (docsUrl: string): ReactNode => <>The token the installer writes stays valid until you revoke it, so a long-running agent keeps working without renewing anything. Config file locations and the manual equivalent are in the <a className="inline-link" href={docsUrl} target="_blank" rel="noopener noreferrer">setup docs<span className="external-icon" aria-hidden="true">↗</span></a>.</>,
...
6. 告诉我需要重启客户端，然后确认这 5 个工具可用：create_session、save_material、search_corpus、get_review_items、mark_mastered。`,
    ...
    step2: "3. 或者跑本地安装器，它问你要 token 时粘贴进去。",
    ...
    hint2b: (docsUrl: string): ReactNode => <>安装器写进配置的 token 在你吊销之前一直有效，所以常驻运行的 agent 不需要续期。各家配置文件的位置和手动写法见 <a className="inline-link" href={docsUrl} target="_blank" rel="noopener noreferrer">配置文档<span className="external-icon" aria-hidden="true">↗</span></a>。</>,
